...
{
    /// <summary>
    /// Recomputes every named-approved representation boundary after scene
    /// serialization. The batch is read-only: it refuses stale visual sources,
    /// recursive dependency drift, prefab or Unity meta mutation, changed GUIDs,
    /// changed 27-role manifest bindings, mixed approval custody, generated roots,
    /// active combat physics, or an arena without a static camera-collision surface.
    /// </summary>
    public static class ActionProductionAssetAuditBatch
    {
        ...
        private const string ReceiptFormat = "rodoh-action-production-asset-audit/2";
        ...
        private sealed class AssetAudit
        {
            ...
            public string markerDependencyClosureSha256;
            public int markerDependencyCount;
            public string computedSourceSha256;
            public string computedDependencyClosureSha256;
            public int computedDependencyCount;
            ...
            public bool exactDependencyCustody;
            public bool exactPrefabCustody;
        ...
        private sealed class Receipt
        {
            ...
            public string productId;
            public string approvedPresentationManifestId;
            public string presentationManifestId;
            ...
            public bool exactDependencyCustody;
            public bool exactPrefabCustody;
            public bool exactBindingCustody;
            public bool exactRepresentationCustody;
            public bool productionApproved;
            public bool generatedPrimitive;
            public bool activePhysicsAuthority;
            public string semanticAuthority = "read-only presentation representation provenance and approval-custody audit";
        ...
        public static void Run()
        {
            ...
            try
            {
                ...
                var exactBindingCustody = approval.declaredBindingClosureSha256 == declared.declaredBindingClosureSha256 && approval.declaredBindingCount == declared.declaredBindingCount && approval.uniqueDeclaredAssetCount == declared.uniqueDeclaredAssetCount;
                if (!exactBindingCustody || declared.declaredBindingCount != 27 || declared.uniqueDeclaredAssetCount != 23)
                {
                    throw new InvalidOperationException("Read-only production audit found representation-binding drift from the named approval.");
                }
                ...
                {
                    Audit(presentation.player.bodyPrefab, profile.player, profile.forbiddenAssetRoots, false, approval, RequiredApproval(approvalById, profile.player.assetId))
                };
                ...
                foreach (var required in profile.enemies)
                {
                    ...
                    assets.Add(Audit(enemy.bodyPrefab, required, profile.forbiddenAssetRoots, false, approval, RequiredApproval(approvalById, required.assetId)));
                }
                assets.Add(Audit(presentation.arena.recipe, profile.arena, profile.forbiddenAssetRoots, true, approval, RequiredApproval(approvalById, profile.arena.assetId)));
                ...
                receipt.approvedPresentationManifestId = approval.presentationManifestId;
                ...
                receipt.exactDependencyCustody = assets.All(value => value.exactDependencyCustody);
                receipt.exactPrefabCustody = assets.All(value => value.exactPrefabCustody);
                receipt.exactBindingCustody = exactBindingCustody;
                receipt.exactRepresentationCustody = assets.All(value => value.exactRepresentationCustody) && exactBindingCustody;
                ...
                {
                    throw new InvalidOperationException("Production asset audit did not retain the complete named-approved seven-asset and 27-binding floor.");
                }
        ...
        private static AssetAudit Audit(string prefabPath, AssetRequirement requirement, string[] forbiddenRoots, bool arena, ApprovalReceipt approvalReceipt, ApprovalAsset approval)
        {
            ...
            var exactSourceCustody = marker.VisualSourceSha256 == closure.visualSourceSha256 && approval.visualSourceSha256 == closure.visualSourceSha256;
            var exactDependencyCustody = marker.DependencyClosureSha256 == closure.dependencyClosureSha256 && approval.dependencyClosureSha256 == closure.dependencyClosureSha256 && marker.DependencyCount == closure.dependencyCount && approval.dependencyCount == closure.dependencyCount;
            var exactPrefabCustody = approval.prefabGuid == prefabGuid && approval.prefabSha256 == prefabSha && approval.prefabMetaSha256 == prefabMetaSha;
            if (!exactSourceCustody) throw new InvalidOperationException("Production asset visual-source digest is stale for " + path + ".");
            if (!exactDependencyCustody) throw new InvalidOperationException("Production asset dependency closure is stale for " + path + ".");
            if (!exactPrefabCustody) throw new InvalidOperationException("Production prefab bytes, Unity meta bytes, or GUID changed after named approval: " + path + ".");
            ...
            return new AssetAudit
            {
                ...
                markerSourceSha256 = marker.VisualSourceSha256,
                markerDependencyClosureSha256 = marker.DependencyClosureSha256,
                markerDependencyCount = marker.DependencyCount,
                computedSourceSha256 = closure.visualSourceSha256,
                computedDependencyClosureSha256 = closure.dependencyClosureSha256,
                computedDependencyCount = closure.dependencyCount,
                ...
                exactDependencyCustody = exactDependencyCustody,
                exactPrefabCustody = exactPrefabCustody,
                exactRepresentationCustody = exactSourceCustody && exactDependencyCustody && exactPrefabCustody,
