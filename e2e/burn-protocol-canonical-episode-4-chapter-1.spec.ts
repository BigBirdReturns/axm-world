...
const ARC_PATH = process.env["BURN_PROTOCOL_THROUGH_EPISODE_4_CHAPTER_1_ARC_PATH"];
...
test.describe("The Burn Protocol Episode 4 Chapter 1", () => {
  test.skip(!ARC_PATH, "Requires the exact Arc publication through Episode 4 Chapter 1.");
  test.describe.configure({ mode: "serial" });

  test("crosses into Osyraa's Offer through the unchanged fixed reader", async ({ page }, testInfo) => {
    test.slow();
    ...
    await expect(page.getByText(/PANEL 181 OF 200/)).toBeVisible();
    await page.screenshot({
      path: testInfo.outputPath(`burn-episode-4-chapter-1-seam-${testInfo.project.name}.png`),
      fullPage: true,
    });

    await advanceRange(page, 4, 1, 2, 10);
    await expect(host).toHaveAttribute("data-panel-id", "E04-C1-P10");
    ...
    await advanceRange(page, 4, 1, 11, 20);
    await expect(host).toHaveAttribute("data-panel-id", "E04-C1-P20");
    await expect(page.getByText(/PANEL 200 OF 200/)).toBeVisible();
    await page.screenshot({
      path: testInfo.outputPath(`burn-episode-4-chapter-1-terminal-${testInfo.project.name}.png`),
      fullPage: true,
    });
    ...
    await expect(host).toHaveAttribute("data-panel-id", "E04-C1-P19");
