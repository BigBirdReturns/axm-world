...
const checksumText = await fsp.readFile(path.join(carrierRoot, 'source.tar.gz.sha256'), 'utf8');
const checksumMatch = /^([a-f0-9]{64})  source\.tar\.gz\s*$/.exec(checksumText);
if (!checksumMatch) throw new Error('Malformed MotionDeck cabinet source checksum.');
const expectedArchiveSha256 = checksumMatch[1];
const partNames = (await fsp.readdir(carrierRoot))
  .filter((name) => /^source\.tar\.gz\.part\d+$/.test(name))
  .sort((a, b) => a.localeCompare(b, 'en-US'));
if (partNames.length === 0) throw new Error('No MotionDeck cabinet binary carrier parts were found.');
const archive = Buffer.concat(await Promise.all(partNames.map((name) => fsp.readFile(path.join(carrierRoot, name)))));
const actualArchiveSha256 = crypto.createHash('sha256').update(archive).digest('hex');
if (actualArchiveSha256 !== expectedArchiveSha256) throw new Error(`MotionDeck cabinet source SHA-256 mismatch: ${actualArchiveSha256}`);

const temporaryRoot = await fsp.mkdtemp(path.join(os.tmpdir(), 'motiondeck-cabinet-materialize-'));
...
try {
  ...
  const expectedPrefix = 'labs/motiondeck-stardew-cabinet-runtime-v0.1.0/';
  for (const rawEntry of listing.split(/\r?\n/)) {
    ...
    if (entry !== expectedPrefix.slice(0, -1) && !entry.startsWith(expectedPrefix)) throw new Error(`Unexpected archive root: ${rawEntry}`);
  }
  run('tar', ['-xzf', archivePath, '-C', extractedRoot]);
  const stagedPackage = path.join(extractedRoot, 'labs', 'motiondeck-stardew-cabinet-runtime-v0.1.0');
  ...
  while (queue.length > 0) {
    ...
    for (const entry of entries) {
      ...
      try { destinationBytes = await fsp.readFile(destination); }
      catch (error) { if (error.code !== 'ENOENT') throw error; }
      if (destinationBytes !== null) {
        if (!sourceBytes.equals(destinationBytes)) throw new Error(`Refusing to overwrite divergent reviewable source: ${relative}`);
        identicalFiles += 1;
        continue;
      }
  ...
  process.stdout.write(`${JSON.stringify({
    format: 'motiondeck-cabinet-source-materialization/1',
    status: 'passed',
    archiveSha256: actualArchiveSha256,
    carrierFormat: 'ordered-binary-parts',
