...
const ARC_PATH = process.env["BURN_PROTOCOL_THROUGH_EPISODE_4_CHAPTER_2_ARC_PATH"];
...
test.describe("The Burn Protocol Episode 4 Chapter 2", () => {
  test.skip(!ARC_PATH, "Requires the exact Arc publication through Episode 4 Chapter 2.");
  test.describe.configure({ mode: "serial" });

  test("crosses into Georgiou's Pattern through the unchanged fixed reader", async ({ page }, testInfo) => {
    test.slow();
    ...
    await expect(page.getByText("220 panel slots", { exact: true })).toBeVisible();
    await expect(page.getByText("44 plate assets", { exact: true })).toBeVisible();
    ...
    await expect(host).toHaveAttribute("data-chapter-id", "E04-C1");
    await expect(page.locator('[data-testid^="canonical-chapter-index-E04-"]')).toHaveCount(2);
    await expect(page.locator('[data-testid^="canonical-panel-index-E04-C1-"]')).toHaveCount(20);
    await advanceRange(page, 4, 1, 2, 20);
    await next(page, "E04-C2-P21");

    await expect(host).toHaveAttribute("data-chapter-id", "E04-C2");
    await expect(page.getByText(
      "Episode 4: Fractured Allegiances · Chapter 2: Georgiou's Pattern",
      { exact: true },
    )).toBeVisible();
    await expect(page.locator('[data-testid^="canonical-chapter-index-E04-"]')).toHaveCount(2);
    await expect(page.locator('[data-testid^="canonical-panel-index-E04-C2-"]')).toHaveCount(20);
    await expect(page.getByTestId("canonical-chapter-index-E04-C2")).toHaveAttribute(
      "data-selected",
      "true",
    );
    ...
    await expect(page.getByText(/PANEL 201 OF 220/)).toBeVisible();
    await page.screenshot({
      path: testInfo.outputPath(`burn-episode-4-chapter-2-seam-${testInfo.project.name}.png`),
      fullPage: true,
    });

    await advanceRange(page, 4, 2, 22, 31);
    await expect(host).toHaveAttribute("data-panel-id", "E04-C2-P31");
    ...
    await advanceRange(page, 4, 2, 32, 40);
    await expect(host).toHaveAttribute("data-panel-id", "E04-C2-P40");
    await expect(page.getByText(/PANEL 220 OF 220/)).toBeVisible();
    await page.screenshot({
      path: testInfo.outputPath(`burn-episode-4-chapter-2-terminal-${testInfo.project.name}.png`),
      fullPage: true,
    });
    ...
    await expect(host).toHaveAttribute("data-panel-id", "E04-C2-P39");
    await expect(page.getByTestId("canonical-story-extent-complete")).toHaveCount(0);

    await page.getByTestId("canonical-chapter-index-E04-C1").click();
    await expect(host).toHaveAttribute("data-panel-id", "E04-C1-P01");
    await page.getByTestId("canonical-chapter-index-E04-C2").click();
    await expect(host).toHaveAttribute("data-panel-id", "E04-C2-P21");
    await page.getByTestId("canonical-story-previous").click();
    await expect(host).toHaveAttribute("data-panel-id", "E04-C1-P20");
    await expect(host).toHaveAttribute("data-chapter-id", "E04-C1");
