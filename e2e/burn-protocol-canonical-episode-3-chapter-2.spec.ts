...
const ARC_PATH = process.env["BURN_PROTOCOL_THROUGH_EPISODE_3_CHAPTER_2_ARC_PATH"];
...
test.describe("The Burn Protocol canonical story through Episode 3 Lockout", () => {
  test.skip(!ARC_PATH, "Requires the exact Arc publication through Episode 3 Chapter 2.");
  test.describe.configure({ mode: "serial" });

  test("crosses into Lockout, preserves P31, and refuses unverifiable media", async ({ page }, testInfo) => {
    test.slow();
    ...
    await expect(page.getByTestId("import-success")).toContainText("Lockout");
    ...
    await expect(page.getByText("160 panel slots", { exact: true })).toBeVisible();
    ...
    await advanceRange(page, 3, 1, 2, 20);
    await next(page, "E03-C2-P21");

    await expect(host).toHaveAttribute("data-chapter-id", "E03-C2");
    await expect(page.getByText(
      "Episode 3: The Omega Thread · Chapter 2: Lockout",
      { exact: true },
    )).toBeVisible();
    await expect(page.locator('[data-testid^="canonical-chapter-index-E03-"]')).toHaveCount(2);
    await expect(page.locator('[data-testid^="canonical-panel-index-E03-C2-"]')).toHaveCount(20);
    await expect(page.getByTestId("canonical-chapter-index-E03-C2")).toHaveAttribute(
      "data-selected",
      "true",
    );
    ...
    await expect(page.getByText(/PANEL 141 OF 160/)).toBeVisible();
    await page.screenshot({
      path: testInfo.outputPath(`burn-lockout-seam-${testInfo.project.name}.png`),
      fullPage: true,
    });

    await advanceRange(page, 3, 2, 22, 31);
    await expect(host).toHaveAttribute("data-panel-id", "E03-C2-P31");
    await expect(page.getByTestId("canonical-panel-asset-source-required-status")).toContainText(
      "SOURCE-REQUIRED ASSET",
    );
    await expect(page.getByTestId("canonical-panel-asset-source-required-status")).toContainText(
      "expected",
    );
    await expect(page.getByTestId("canonical-panel-asset-source-required")).toContainText(
      "Exact asset receipt required",
    );
    await expect(page.getByTestId("canonical-panel-asset-source-required")).toContainText(
      "a03c2-art-manifest",
    );
    await expect(page.getByTestId("canonical-panel-image")).toHaveCount(0);
    await expectNoHorizontalOverflow(page);
    await page.screenshot({
      path: testInfo.outputPath(`burn-lockout-p31-source-required-${testInfo.project.name}.png`),
      fullPage: true,
    });
    ...
    await expect(page.getByTestId("canonical-panel-asset-source-required")).toBeVisible();

    await page.getByTestId("canonical-story-file-input").setInputFiles({
      name: "E03-C2-P31.webp",
      mimeType: "image/webp",
      buffer: Buffer.alloc(156_208),
    });
    await expect(page.getByTestId("canonical-story-asset-errors")).toContainText(
      "exact asset receipt is source-required",
    );
    await expect(page.getByTestId("canonical-panel-image")).toHaveCount(0);

    await advanceRange(page, 3, 2, 32, 40);
    await expect(host).toHaveAttribute("data-panel-id", "E03-C2-P40");
    await page.screenshot({
      path: testInfo.outputPath(`burn-through-lockout-terminal-${testInfo.project.name}.png`),
      fullPage: true,
    });
    ...
    await expect(host).toHaveAttribute("data-panel-id", "E03-C2-P39");
    await expect(page.getByTestId("canonical-story-extent-complete")).toHaveCount(0);

    await page.getByTestId("canonical-chapter-index-E03-C2").click();
    await expect(host).toHaveAttribute("data-panel-id", "E03-C2-P21");
    await page.getByTestId("canonical-story-previous").click();
