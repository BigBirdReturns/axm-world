...
function fixture(): CanonicalStorySource {
  return parseCanonicalStory({
    ...
    identity: {
      id: "episode-4-chapter-2-story",
      title: "Episode 4 Chapter 2 Story",
      version: "0.9.0",
    },
...
describe("World Episode 4 Chapter 2 receiver law", () => {
  it("crosses the Episode 4 chapter seam in both directions", () => {
    ...
    if (forward.kind !== "panel") throw new Error("Expected Georgiou's Pattern opening.");
    expect(forward.cursor).toEqual({
      storyId: "episode-4-chapter-2-story",
      episodeId: "E04",
      chapterId: "E04-C2",
      panelId: "E04-C2-P21",
    });
  ...
  it("reports Episode 4 Chapter 3 as the sole outside continuation", () => {
    const story = fixture();
    ...
    expect(advanceCanonicalStory(story, terminal)).toEqual({
      ...
      cursor: terminal,
      continuationPanelId: "E04-C3-P41",
    });
  });

  it("preserves the inherited P31 media boundary under the two-chapter Episode 4 data", () => {
    const story = fixture();
    ...
    expect(story.episodes[3]!.chapters.map((candidate) => candidate.title)).toEqual([
      "Osyraa's Offer",
      "Georgiou's Pattern",
    ]);
  });
