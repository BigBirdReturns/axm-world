...
{
    /// <summary>
    /// Production safety adapter for ActionSessionSpoolRuntime. ActionSafetyGate
    /// writes the full observation first, then invokes this adapter with the reason.
    /// </summary>
    [DefaultExecutionOrder(11000)]
    public sealed class ActionSafetySpoolRuntimeAdapter : MonoBehaviour
    {
        ...
        private void OnBreach(string reason)
        {
            if (spool == null)
            {
                Debug.LogError("Action safety observation could not enter the production spool because ActionSessionSpoolRuntime is absent.");
                return;
            }
            try
            {
