...
const ARC_PATH = process.env["BURN_PROTOCOL_THROUGH_EPISODE_4_ARC_PATH"];
...
test.describe("The Burn Protocol complete Episode 4", () => {
  test.skip(!ARC_PATH, "Requires the exact complete Episode 4 Arc publication.");
  test.describe.configure({ mode: "serial" });

  test("crosses into The Dead Man's Checksum through the unchanged fixed reader", async ({ page }, testInfo) => {
    test.slow();
    ...
    await expect(page.getByText("240 panel slots", { exact: true })).toBeVisible();
    await expect(page.getByText("48 plate assets", { exact: true })).toBeVisible();
    ...
    await expect(page.locator('[data-testid^="canonical-chapter-index-E04-"]')).toHaveCount(3);
    await page.getByTestId("canonical-chapter-index-E04-C2").click();
    await expect(host).toHaveAttribute("data-panel-id", "E04-C2-P21");
    await advanceRange(page, 4, 2, 22, 40);
    await next(page, "E04-C3-P41");

    await expect(host).toHaveAttribute("data-chapter-id", "E04-C3");
    await expect(page.getByText(
      "Episode 4: Fractured Allegiances · Chapter 3: The Dead Man's Checksum",
      { exact: true },
    )).toBeVisible();
    await expect(page.locator('[data-testid^="canonical-chapter-index-E04-"]')).toHaveCount(3);
    await expect(page.locator('[data-testid^="canonical-panel-index-E04-C3-"]')).toHaveCount(20);
    await expect(page.getByTestId("canonical-chapter-index-E04-C3")).toHaveAttribute(
      "data-selected",
      "true",
    );
    ...
    await expect(page.getByText(/PANEL 221 OF 240/)).toBeVisible();
    await page.screenshot({
      path: testInfo.outputPath(`burn-episode-4-chapter-3-seam-${testInfo.project.name}.png`),
      fullPage: true,
    });

    await advanceRange(page, 4, 3, 42, 50);
    await expect(host).toHaveAttribute("data-panel-id", "E04-C3-P50");
    ...
    await advanceRange(page, 4, 3, 51, 60);
    await expect(host).toHaveAttribute("data-panel-id", "E04-C3-P60");
    await expect(page.getByText(/PANEL 240 OF 240/)).toBeVisible();
    await page.screenshot({
      path: testInfo.outputPath(`burn-episode-4-terminal-${testInfo.project.name}.png`),
      fullPage: true,
    });
    ...
    await expect(host).toHaveAttribute("data-panel-id", "E04-C3-P59");
    ...
    await expect(host).toHaveAttribute("data-panel-id", "E04-C3-P41");
    await page.getByTestId("canonical-story-previous").click();
