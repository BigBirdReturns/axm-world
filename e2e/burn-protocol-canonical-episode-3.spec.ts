...
const ARC_PATH = process.env["BURN_PROTOCOL_THROUGH_EPISODE_3_ARC_PATH"];
...
test.describe("The Burn Protocol complete canonical Episode 3", () => {
  test.skip(!ARC_PATH, "Requires the exact complete Episode 3 Arc publication.");
  test.describe.configure({ mode: "serial" });

  test("crosses into Prime Incident and reaches the Episode 4 continuation", async ({ page }, testInfo) => {
    test.slow();
    ...
    await expect(page.getByTestId("import-success")).toContainText("The Omega Thread");
    ...
    await expect(page.getByText("180 panel slots", { exact: true })).toBeVisible();
    await expect(page.getByText("36 plate assets", { exact: true })).toBeVisible();
    ...
    await advanceRange(page, 3, 2, 22, 40);
    await next(page, "E03-C3-P41");

    await expect(host).toHaveAttribute("data-chapter-id", "E03-C3");
    await expect(page.getByText(
      "Episode 3: The Omega Thread · Chapter 3: Prime Incident",
      { exact: true },
    )).toBeVisible();
    await expect(page.locator('[data-testid^="canonical-chapter-index-E03-"]')).toHaveCount(3);
    await expect(page.locator('[data-testid^="canonical-panel-index-E03-C3-"]')).toHaveCount(20);
    await expect(page.getByTestId("canonical-chapter-index-E03-C3")).toHaveAttribute(
      "data-selected",
      "true",
    );
    ...
    await expect(page.getByText(/PANEL 161 OF 180/)).toBeVisible();
    await page.screenshot({
      path: testInfo.outputPath(`burn-prime-incident-seam-${testInfo.project.name}.png`),
      fullPage: true,
    });

    await advanceRange(page, 3, 3, 42, 50);
    await expect(host).toHaveAttribute("data-panel-id", "E03-C3-P50");
    ...
    await advanceRange(page, 3, 3, 51, 60);
    await expect(host).toHaveAttribute("data-panel-id", "E03-C3-P60");
    await expect(page.getByText(/PANEL 180 OF 180/)).toBeVisible();
    await page.screenshot({
      path: testInfo.outputPath(`burn-episode-3-terminal-${testInfo.project.name}.png`),
      fullPage: true,
    });
    ...
    await expect(host).toHaveAttribute("data-panel-id", "E03-C3-P59");
    ...
    await expect(host).toHaveAttribute("data-panel-id", "E03-C3-P41");
    await page.getByTestId("canonical-story-previous").click();
